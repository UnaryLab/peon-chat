...
# (the service manager holds an O_APPEND fd, so rename-based rotation would not work), keep 5 archives.
LOG="$(cd "$(dirname "$0")/.." && pwd)/peon-chat.log"
[ -f "$LOG" ] || exit 0
[ "$(wc -c < "$LOG")" -gt 10485760 ] || exit 0
